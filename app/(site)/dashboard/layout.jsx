      ...
      <div>
                  ...
                  <div className="flex grow flex-col gap-y-5 overflow-y-auto bg-white px-6 pb-4">
                    ...
                    <nav className="flex flex-1 flex-col">
                      <ul role="list" className="flex flex-1 flex-col gap-y-7">
                                ...
                                <Link
                                  ...
                                  className={classNames(
                                    item.current
                                      ...
                                      : 'text-charcoal-700 hover:text-flame-600 hover:bg-charcoal-50',
                                    'group flex gap-x-3 rounded-md p-2 text-sm leading-6 font-semibold'
                                  )}
                                >
                                  <item.icon
                                    className={classNames(
                                      item.current ? 'text-flame-600' : 'text-charcoal-400 group-hover:text-flame-600',
                                      'h-6 w-6 shrink-0'
                                    )}
                                    aria-hidden="true"
                                  />
                        ...
                        <li>
                          <div className="text-xs font-semibold leading-6 text-charcoal-400">Your teams</div>
                              ...
                              <li key={team.name}>
                                ...
                                >
                                  <span
                                    className={classNames(
                                      team.current
                                        ? 'text-flame-600 border-flame-600'
                                        : 'text-charcoal-400 border-charcoal-200 group-hover:border-flame-600 group-hover:text-flame-600',
                        ...
                        <Link
                          ...
                          className={classNames(
                            item.current
                              ...
                              : 'text-charcoal-700 hover:text-flame-800 hover:bg-charcoal-50',
                            'group flex gap-x-3 rounded-md p-2 text-sm leading-6 font-semibold'
                          )}
                        >
                          <item.icon
                            className={classNames(
                              item.current ? 'text-flame-800' : 'text-timberwolf-600 group-hover:text-flame-800',
                              'h-6 w-6 shrink-0'
                            )}
                            aria-hidden="true"
                          />
        ...
        <div className="lg:pl-72">
          <div className="sticky top-0 z-40 flex h-16 shrink-0 items-center gap-x-4 border-b border-charcoal-200 bg-white px-4 shadow-sm sm:gap-x-6 sm:px-6 lg:px-8">
            <button type="button" className="-m-2.5 p-2.5 text-charcoal-700 lg:hidden" onClick={() => setSidebarOpen(true)}>
              <span className="sr-only">Open sidebar</span>
              <Bars3Icon className="h-6 w-6" aria-hidden="true" />
            </button>

            {/* Separator */}
            <div className="h-6 w-px bg-charcoal-200 lg:hidden" aria-hidden="true" />
            ...
            <div className="flex flex-1 gap-x-4 self-stretch lg:gap-x-6">
              ...
              <div className="flex items-center gap-x-4 lg:gap-x-6">
                <button type="button" className="-m-2.5 p-2.5 text-charcoal-400 hover:text-charcoal-500">
                  <span className="sr-only">View notifications</span>
                  <BellIcon className="h-6 w-6" aria-hidden="true" />
                </button>

                {/* Separator */}
                <div className="hidden lg:block lg:h-6 lg:w-px lg:bg-charcoal-200" aria-hidden="true" />
                ...
                <Menu as="div" className="relative">
                  <Menu.Button className="-m-1.5 flex items-center p-1.5">
                    ...
                    {!session? <HiMiniUser size={"36px"} className='h8 w-8' /> : (
                    <Image
                    ...
                    <span className="hidden lg:flex lg:items-center">
                      <span className="ml-4 text-sm font-semibold leading-6 text-charcoal-900" aria-hidden="true">
                        {!session? "Loading..." : (
                        session.user.name
                        )}
                      </span>
                      <ChevronDownIcon className="ml-2 h-5 w-5 text-charcoal-400" aria-hidden="true" />
                    </span>
                  </Menu.Button>
                  <Transition
                    as={Fragment}
